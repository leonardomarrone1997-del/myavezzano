...
let mapPlaces = businesses.concat(userSubmittedBusinesses, [
  ...
  },
  {
    ...
    stats: "Teatro Off",
    image: "https://images.unsplash.com/photo-1503095396549-807759245b35?auto=format&fit=crop&w=500&q=80",
    caption: "Giardino del Castello Orsini per cabaret, teatro contemporaneo e appuntamenti culturali."
  },
  {
    name: "Anfiteatro Romano di Alba Fucens",
    category: "Eventi",
    distance: "Alba Fucens",
    lat: 42.0775,
    lng: 13.4130,
    address: "Alba Fucens, Massa d'Albe",
    stats: "Festiv'Alba 2026",
    image: "https://images.unsplash.com/photo-1531572753322-ad063cecc140?auto=format&fit=crop&w=500&q=80",
    caption: "Cornice archeologica per musica, teatro classico e spettacoli estivi."
  },
  {
    name: "Teatro dei Marsi",
    category: "Eventi",
    distance: "Avezzano",
    lat: 42.0358,
    lng: 13.4215,
    address: "Avezzano",
    stats: "Backup maltempo",
    image: "https://images.unsplash.com/photo-1508973379184-7517410fb0bc?auto=format&fit=crop&w=500&q=80",
    caption: "Sede alternativa indicata per gli spettacoli in caso di maltempo."
  },
  {
    name: "Libreria Ubik",
    category: "Servizio",
...
function spawnSwallow() {
  ...
  const flockSize = compact ? 3 + Math.floor(Math.random() * 2) : 4 + Math.floor(Math.random() * 2);
  const baseDirection = Math.random() > 0.28 ? 1 : -1;
  const baseY = compact ? 10 + Math.random() * 10 : 8 + Math.random() * 16;
  const baseDuration = compact ? 17 + Math.random() * 5 : 15 + Math.random() * 6;
  ...
  Array.from({ length: flockSize }).forEach((_, index) => {
    ...
    bird.style.setProperty("--fly-opacity", `${compact ? 0.28 + Math.random() * 0.16 : 0.38 + Math.random() * 0.22}`);
    bird.style.setProperty("--wing-speed", `${compact ? 0.58 + Math.random() * 0.18 : 0.46 + Math.random() * 0.18}s`);
...
function scheduleSwallow(initial = false) {
  ...
  scheduleAtmosphere(
    spawnSwallow,
    initial ? (compact ? 4200 : 2600) : (compact ? 36000 : 22000),
    initial ? (compact ? 9000 : 6400) : (compact ? 68000 : 46000)
  );
}
...
function scheduleShootingStar(initial = false) {
  ...
  scheduleAtmosphere(
    spawnShootingStar,
    initial ? (compact ? 7000 : 3400) : (compact ? 46000 : 26000),
    initial ? (compact ? 14000 : 9000) : (compact ? 82000 : 52000)
  );
}
...
function applySearchFilter() {
  ...
  if (searchStatus) {
    ...
    const visibleTerm = searchInput?.value.trim() || term;
    searchStatus.textContent = activeResults.length
      ? `${activeResults.length} risultati per "${visibleTerm}"`
      : `Non ho trovato nulla per "${visibleTerm}". Prova con una categoria o un luogo vicino.`;
  }
...
document.addEventListener("click", (event) => {
  ...
  const searchInput = document.querySelector("#searchInput");
  if (!searchInput) {
    if (!document.querySelector("#mapView").classList.contains("active")) switchView("map");
    showToast(`Apro la mappa per: ${quickSearch.dataset.searchTerm}`, "success");
    return;
  }
