...
Build statica per Vercel:

```bash
npm run build
```

La build copia la PWA in `public/`. Su Vercel l'Output Directory corretta e `public`.
...
- `framework` su `null`, cioe preset Vercel "Other".
- `buildCommand` su `npm run build`.
- `outputDirectory` su `public`.
